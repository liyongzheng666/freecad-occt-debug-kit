...
| G30 | **假绿判别决策空间**（IsDone=true+invalid 的根因细分；"决策空间大"A5 残留） | ✅ **2026-07-02③ 落地**：playbook 签名② `fillet-falsegreen-invalid`（S2-bsurf/S3-中段自交/S4-端盖 三候选，走同一 decide 接缝）+ `falsegreen_probe`（支撑面类型/缺陷端局部性 d_end≤0.1/自由端前置）。四锚点实证：E7 fired-S4（d_end=0.000）/E8 fired-S2（BSpline）/thinplate S3 兜回归/E4 全排除兜底。⚠ LLM replay 对新签名无录制 → llm 臂假绿 case SKIP（待重录） | 🟠 | A5/P2 |
