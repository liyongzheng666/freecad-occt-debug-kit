...
| `README.md` | 本文件 |
| `DISPLAY_RULES.md` | **显示约束**（颜色/kind/含义/失败三态），以本 demo 为范例固化 |
...
| 位置 | 场景 | 表征 | 失败态 |
| --- | --- | --- | --- |
| x≈0–10 | **凸**：方块外棱 90° | 🟠 真实圆角面（makeFillet 圆柱面），**削棱**；法向岔开 | ① 成功 |
| x≈20–40 | **凹**：L 形内角 90° | 🟠 真实圆角面，**填角**；法向对冲 | ① 成功 |
| x≈50–70 | 窄槽宽 4，r=3 | 🔴+🟠 两圆角面**重叠穿插** | ② 算法放弃·**可 SSI 互裁** |
| x≈80–96 | 盲孔 R=3，r=4 | 🔴 **滚球 r=4 > 孔 R=3**，塞不进、无有效圆角面 | ③ 几何**真·不可能** |

其余：青/蓝=两支撑面、红线=共享棱、黄箭头=两面外法向。**颜色/含义见 `DISPLAY_RULES.md`（已固化为约束）。**
...
**真正的"几何不可能 / 球塞不进"** 是 **r > 局部凹曲率半径**：见**第四场景**（x≈80–96）——盲孔半径 ρ=3，r=4>ρ 时滚球比孔还大、塞不进孔底内角，**根本不存在**有效圆角面（SSI 互裁也没有面可裁）；实测 r≤3 能成、r=4 抛 `StdFail_NotDone`。这才是降半径之外无解的硬失败，与第三例（算法放弃·可裁剪）形成对照。

> 这三态（成功 / 算法放弃·可裁剪 / 几何不可能）正是 root-cause agent 对 `StdFail_NotDone` 该区分的结论：别默认"降半径"，先判是"可裁剪(算法)"还是"不可能(几何)"。详见 `DISPLAY_RULES.md` §3。
